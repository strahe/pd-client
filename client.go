		...
		go func(dc string, tsDeadlineCh <-chan deadline) {
			for {
				...
				case d := <-tsDeadlineCh:
					select {
					case <-d.timer:
						log.Error("tso request is canceled due to timeout", zap.String("dc-location", dc))
						d.cancel()
					case <-d.done:
					case <-ctx.Done():
						return
					...
					for {
						...
						if stream == nil {
							...
							if err != nil {
								...
								log.Error("[pd] create tso stream error", zap.String("dc-location", dc))
								c.ScheduleCheckLeader()
								cancel()
						...
						if err != nil {
							...
							log.Error("[pd] getTS error", zap.String("dc-location", dc))
							c.ScheduleCheckLeader()
							cancel()
							stream = nil
						}
...
func (c *client) dispatchRequest(dcLocation string, request *tsoRequest) *tsoRequest {
	...
	if !ok {
		err := xerrors.New(fmt.Sprintf("unknown dc-location %s to the client", dcLocation))
		log.Error("[pd] dispatch tso request error")
		request.done <- err
		return request
	}
...
func (c *client) GetRegionFromMember(ctx context.Context, key []byte, memberURLs []string) (*Region, error) {
	...
	for _, url := range memberURLs {
		...
		if err != nil {
			log.Error("[pd] can't get grpc connection", zap.String("member-URL", url))
			continue
		}
		...
		if err != nil {
			log.Error("[pd] can't get region info", zap.String("member-URL", url))
			continue
		}
		if resp != nil {
			break
