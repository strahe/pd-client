...
func (c *baseClient) leaderLoop() {
	...
	for {
		...
		if err := c.updateLeader(); err != nil {
			log.Error("[pd] failed updateLeader")
		}
	}
}
...
func (c *baseClient) initClusterID() error {
	...
	for _, u := range c.urls {
		...
		if err != nil || members.GetHeader() == nil {
			log.Warn("[pd] failed to get cluster id", zap.String("url", u))
			continue
		}
...
func (c *baseClient) updateLeader() error {
	for _, u := range c.urls {
		...
		if err != nil {
			log.Warn("[pd] cannot update leader", zap.String("address", u))
		}
		cancel()
	...
	return xerrors.New("ErrClientGetLeader")
}
